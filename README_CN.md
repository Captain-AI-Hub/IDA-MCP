...
`open_in_ida` 是 proxy 侧的生命周期工具。它会使用 `IDA_PATH` 或 `config.conf` 中的 `ida_path` 解析 IDA 可执行文件，并通过子进程环境变量 `IDA_MCP_AUTO_START=1` 和预留好的 `IDA_MCP_PORT` 请求插件在正常 GUI 模式下自动启动。
